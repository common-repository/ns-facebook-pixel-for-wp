 ...
 *
 * TODO Search & Replace fbp4wp_ with your prefix
 * TODO Search & Replace Fbp4wp_ with your prefix
 * TODO Search & Replace 'ns-facebook-pixel-for-wp' with your 'ns-facebook-pixel-for-wp'
...
class Fbp4wp_Modules_Reviews {
	 ...
	 * @var string
	 */
	public static $api_url = 'https://revieworgapi.nsthemes.com/wp-json/fbp4wp/v1/review_action';
	...
	public static function init() {
		...
		add_action( 'wp_ajax_fbp4wp_review_action', array( __CLASS__, 'ajax_handler' ) );
	}
	...
	public static function installed_on() {
		$installed_on = get_option( 'fbp4wp_reviews_installed_on', false );

		if ( ! $installed_on ) {
			$installed_on = current_time( 'mysql' );
			update_option( 'fbp4wp_reviews_installed_on', $installed_on );
		}
	...
	public static function ajax_handler() {
		...
		if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'fbp4wp_review_action' ) ) {
			wp_send_json_error();
		}
		...
		try {
			...
			update_user_meta( $user_id, '_fbp4wp_reviews_dismissed_triggers', $dismissed_triggers );
			update_user_meta( $user_id, '_fbp4wp_reviews_last_dismissed', current_time( 'mysql' ) );
	...
	public static function dismissed_triggers() {
		...
		$dismissed_triggers = get_user_meta( $user_id, '_fbp4wp_reviews_dismissed_triggers', true );
	...
	public static function already_did( $set = false ) {
		...
		if ( $set ) {
			update_user_meta( $user_id, '_fbp4wp_reviews_already_did', true );

			return true;
		}

		return (bool) get_user_meta( $user_id, '_fbp4wp_reviews_already_did', true );
	}
	...
	public static function triggers( $group = null, $code = null ) {
		...
		if ( ! isset( $triggers ) ) {
			...
			$open_message = __( 'Hey, I noticed you recently hit %d social pixel on your site – that’s awesome!. I would very much appreciate if you could quickly give it a 5-star rating on WordPress, just to help us spread the word.', 'ns-facebook-pixel-for-wp' );
			$time_message = __( "Hey, you've been using NS Tracking Social Pixel for %s on your site - I hope that its been helpful. I would very much appreciate if you could quickly give it a 5-star rating on WordPress, just to help us spread the word.", 'ns-facebook-pixel-for-wp' );

			$triggers = apply_filters( 'fbp4wp_reviews_triggers', array(
				'time_installed' => array(
					'triggers' => array(
						'one_week'     => array(
							'message'    => sprintf( $time_message, __( '1 week', 'ns-facebook-pixel-for-wp' ) ),
							'conditions' => array(
								strtotime( self::installed_on() . ' +1 week' ) < time(),
							),
							'link'       => 'https://wordpress.org/support/plugin/ns-facebook-pixel-for-wp/reviews/?rate=5#rate-response',
							'pri'        => 10,
						),
						'one_month'    => array(
							'message'    => sprintf( $time_message, __( '1 month', 'ns-facebook-pixel-for-wp' ) ),
							'conditions' => array(
						...
						'three_months' => array(
							'message'    => sprintf( $time_message, __( '3 months', 'ns-facebook-pixel-for-wp' ) ),
							'conditions' => array(
				...
				'open_count'     => array(
					'triggers' => array(
						'50_opens'  => array(
							...
							'conditions' => array(
								get_option( 'fbp4wp_total_open_count', 0 ) > 50,
							),
							'link'       => 'https://wordpress.org/support/plugin/ns-facebook-pixel-for-wp/reviews/?rate=5#rate-response',
							'pri'        => 10,
						),
						'100_opens' => array(
							...
							'conditions' => array(
								get_option( 'fbp4wp_total_open_count', 0 ) > 100,
						...
						'500_opens' => array(
							...
							'conditions' => array(
								get_option( 'fbp4wp_total_open_count', 0 ) > 500,
							),
							'link'       => 'https://wordpress.org/support/plugin/ns-facebook-pixel-for-wp/reviews/?rate=5#rate-response',
							'pri'        => 30,
						),
	...
	public static function admin_notices() {
			...
			(function ($) {
				...
				function dismiss(reason) {
					$.ajax({
						...
						url: ajaxurl,
						data: {
							action: 'fbp4wp_review_action',
							nonce: '<?php echo wp_create_nonce( 'fbp4wp_review_action' ); ?>',
							group: trigger.group,
							code: trigger.code,
							pri: trigger.pri,
							reason: reason
		...
		<div class="notice notice-success is-dismissible prefix-notice">
			...
			<p>
				<img class="logo" src="https://ps.w.org/ns-facebook-pixel-for-wp/assets/icon-128x128.png" />
				<strong>
			...
			<ul>
				<li>
					<a class="prefix-dismiss" target="_blank" href="https://wordpress.org/support/plugin/ns-facebook-pixel-for-wp/reviews/?rate=5#rate-response" data-reason="am_now">
						<strong><?php _e( 'Ok, you deserve it', 'ns-facebook-pixel-for-wp' ); ?></strong>
					</a>
				</li>
				<li>
					<a href="#" class="prefix-dismiss" data-reason="maybe_later">
						<?php _e( 'Nope, maybe later', 'ns-facebook-pixel-for-wp' ); ?>
					</a>
				</li>
				<li>
					<a href="#" class="prefix-dismiss" data-reason="already_did">
						<?php _e( 'I already did', 'ns-facebook-pixel-for-wp' ); ?>
					</a>
				</li>
			</ul>
	...
	public static function last_dismissed() {
		...
		return get_user_meta( $user_id, '_fbp4wp_reviews_last_dismissed', true );
	}
